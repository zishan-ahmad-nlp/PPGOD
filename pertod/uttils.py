...
def add_special_tokens():
	...
	# special_tokens = {'pad_token':'<|pad|>','sep_token':'<|sep|>','additional_special_tokens':['<|startip|>','<|endip|>','<|startop|>','<|endop|>','<|startent|>','<|endent|>','<|startone|>','<|endone|>','<|starttwo|>','<|endtwo|>','<|startthr|>','<|endthr|>','<|startfour|>','<|endfour|>','<|startfiv|>','<|endfiv|>']}#,'end_token':'<|eot|>','beg_token':'<|boo|>'}#}#{'pad_token':'<|pad|>','sep_token':'<|sep|>'}
...
def generate_beam_sample(data, tokenizer, model, num=1, length=60, beam_size=3, device=torch.device('cuda')):
...
def generate_sample(data, tokenizer, model, model_checkpoint, num=1, length=120, train=False, temperature=1, top_k=10, top_p=0.5, device=torch.device('cuda')):
    ...
    summary_list = []
    gen_list = []
    f_out = open(f'{model_checkpoint}/test_output.txt', 'w')
    ...
    for i in range(num):
        ...
        try:
            ...
            text = tokenizer.convert_tokens_to_string(text)
            print(text)
            index = text.find('<|endofresponse|>')
            f_out.write(text[:index].strip()+'\n')
        except:
            f_out.write(' '+'\n')
        # article = tokenizer.decode(context).replace('<|pad|>', '')
        # summary = tokenizer.decode(summary).replace('<|sep|>', '').replace('<|endip|>','').replace('<|startop|>','').replace('<|endop|>','').replace('<|startone|>', '').replace('<|endone|>', '').replace('<|starttwo|>', '').replace('<|endtwo|>', '').replace('<|startthr|>', '').replace('<|endthr|>', '').replace('<|startfour|>', '').replace('<|endfour|>', '').replace('<|startfiv|>', '').replace('<|endfiv|>', '')
    #     wordlist1 = (summary).split()
        
    #     reverse_text = text[::-1]
    #     remove_text = ''
    #     # if (reverse_text[0]=='.') or (reverse_text[0]=='!'):
    #     #     pass
    #     # else:
    #     for j in range(len(reverse_text)):
    #         if (reverse_text[j]=='.') or (reverse_text[j]=='!'):
    #             break
    #         else:
    #             remove_text+=reverse_text[j]
    #     remove_text = remove_text[::-1]
    #     new_text = text.replace(remove_text,'')
    #     print(len(new_text))
    #     if (len(new_text) == 0) or (len(new_text) <= 135):
    #         new_text = text

    #     word = '<|pad|>'
    #     if word in wordlist1:
    #         newsummary = [x for x in wordlist1 if x not in word]
    #         newsummary = ' '.join(newsummary)
    #     #substring = "<|startent|>"
    #     #newsummary  = newsummary[:newsummary.index(substring)]
    #     df.append([article, new_text, newsummary])
    #     article_list.append(article)
    #     summary_list.append(newsummary)
    #     gen_list.append(new_text)
    #     # print('Article: ', end='\n\n')
    #     # print(article, end='\n\n')
    #     # print("Generated_Oneliner: ", end='\n\n')
    #     # print(text, end='\n\n')
    #     # print('Actual_Oneliner: ', end='\n\n')
    #     # print(newsummary, end='\n\n')
    # if not train:
    #     with open('./output/epoch5/article_list.txt', 'w') as f:
    #         for item in article_list:
    #             f.write("%s\n" % item)
    #     with open('./output/epoch5/summary_list.txt', 'w') as f:
    #         for item in summary_list:
    #             f.write("%s\n" % item)
    #     with open('./output/epoch5/gen_list.txt', 'w') as f:
    #         for item in gen_list:
    #             f.write("%s\n" % item)
    #     df = pd.DataFrame(df, columns = ['Article', 'Generated Oneliner', 'Original Oneliner'])
    #     df.to_excel("./output/epoch5/generated.xlsx", index=False)
